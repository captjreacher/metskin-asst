...
// - Uses Responses API with file_search + vector store IDs
...
if (VECTOR_STORE_IDS.length === 0) console.warn('[BOOT] No vector stores set. Add VECTOR_STORE_ID or VECTOR_STORE_IDS.');
...
const ASST_INSTRUCTIONS = process.env.ASST_INSTRUCTIONS || 'You are the Metamorphosis Assistant. Use the knowledge base (file_search) to answer accurately. Be concise.';
...
app.use((err, _req, res, next) => {
  if (err && err.type === 'entity.parse.failed') return res.status(400).json({ ok: false, error: 'Invalid JSON payload', details: String(err.message || '') });
  next(err);
});
...
const headersForLog = (h) => ({ Authorization: h.Authorization ? `Bearer ${h.Authorization.slice(7, 11)}…` : '(missing)', 'OpenAI-Beta': h['OpenAI-Beta'] || '(none)', 'Content-Type': h['Content-Type'] });
const blocks = (text) => [{ role: 'user', content: [{ type: 'input_text', text: String(text ?? '') }] }];
const withKnowledge = (payload) => VECTOR_STORE_IDS.length ? { ...payload, tools: [{ type: 'file_search' }], tool_resources: { file_search: { vector_store_ids: VECTOR_STORE_IDS } } } : payload;
...
async function callResponses(body, { timeoutMs = 45_000 } = {}) {
  ...
  if (DBG_OA) { console.log('[OA HEADERS]', headersForLog(OA_HEADERS)); console.log('[OA⇢]', RESPONSES_URL, '\n' + JSON.stringify(body, null, 2)); }
  let resp, raw; try { resp = await fetch(RESPONSES_URL, req); raw = await resp.text(); } finally { clearTimeout(timer); }
...
app.post('/assistant/ask', async (req, res) => {
  try {
    ...
    const payload = withKnowledge({ model: model || DEFAULT_MODEL, instructions: ASST_INSTRUCTIONS, input: blocks(userText), store: true, ...(prior ? { previous_response_id: prior } : {}), ...(USE_ASSISTANT && ASST_DEFAULT ? { assistant_id: ASST_DEFAULT } : {}) });
